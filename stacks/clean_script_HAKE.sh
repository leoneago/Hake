...
clean=/share/projects/HAKE/clean/$1

temp=(`grep -v "#" $clean/$1.txt  |wc`)
n=(`expr $temp - 1`)

read1=(`grep -w $1 ruta.txt |cut -f 3`)
read2=(`grep -w $1 ruta.txt |cut -f 4`)
...
process_radtags2 -i gzfastq -1 $read1 -2 $read2 -o $clean/ -e sbfI -b $clean/barcodes.txt -c -q -r -t 90 
...
for i in `seq 0 $n`; do
        ...
        c="$(grep -c ^@._ $clean/sample_${barcodes[i]}.1.fq)"
        echo -n $c >> $clean/sample_reads.txt
        echo -n " " >> $clean/sample_reads.txt
        c="$(zgrep -c ^@._ $clean/${names[i]}.fq_1.gz)"
